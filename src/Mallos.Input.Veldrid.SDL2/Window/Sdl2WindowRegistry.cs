namespace Mallos.Input.Window
{
    ...
    internal static class Sdl2WindowRegistry
    {
        ...
        private static readonly Dictionary<uint, MallosSdl2Window> _eventsByWindowID
            = new Dictionary<uint, MallosSdl2Window>();
        private static bool _firstInit;

        public static void RegisterWindow(MallosSdl2Window window)
        {
            lock (Lock)
            {
        ...
        public static void RemoveWindow(MallosSdl2Window window)
        {
            lock (Lock)
            {
        ...
        private static unsafe void ProcessWindowEvent(ref SDL_Event ev)
        {
            ...
            if (handled && _eventsByWindowID.TryGetValue(windowID, out MallosSdl2Window window))
            {
                window.AddEvent(ev);
            }
        }
